...
#read the csv file
data = pd.read_csv('D:/SemesterProject/Radiation_Record/merge_panel_radiation_ClearSky.csv',header=0)
...
plotData = Data.drop(['Look up Group'],axis=1)
#print(plotData)

totalData = plotData.iloc[range(1,340),:]
#print(summerData)
# sums up all radiation according to day hour
sum_up = totalData.groupby('Hour').sum()
sum_up = sum_up*17
print(sum_up)
...
# Major scale lines
axis_list = np.arange(start=0,stop=300,step=20)
...
# Give a title
plt.title('Annual Total Radiation Analysis by Hour (Clear Sky) (kWh)')
